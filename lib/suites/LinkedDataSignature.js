...
const jsonld = require('@interop/jsonld');
...
/**
 * Shared @interop ecosystem types, imported so the JSDoc type names below
 * resolve to their definitions in `@interop/data-integrity-core`.
 *
 * @typedef {import('@interop/data-integrity-core').AbstractKeyPair} AbstractKeyPair
 * @typedef {import('@interop/data-integrity-core').ISigner} ISigner
 * @typedef {import('@interop/data-integrity-core').IVerifier} IVerifier
 * @typedef {import('@interop/data-integrity-core').IVerificationMethod} IVerificationMethod
 * @typedef {new (...args: any[]) => AbstractKeyPair} LDKeyClass
...
module.exports = class LinkedDataSignature extends LinkedDataProof {
   ...
   * @param {LDKeyClass} options.LDKeyClass - The key pair class
   *   (an `AbstractKeyPair` subclass) that this suite will use to sign/verify
   *   signatures. Provided by subclass. Used during the `verifySignature`
   *   operation, to create an instance (containing a `verifier()` property) of
   *   a public key fetched via a `documentLoader`.
   *
   ...
   *
   * @param {AbstractKeyPair} [options.key] - An optional key pair instance
   *   (containing an `id` property, and either `signer` or `verifier`,
   *   depending on the intended operation. Useful for when the application is
   *   managing keys itself (when using a KMS, you never have access to the
   *   private key, and so should use the `signer` param instead).
   *
   * @param {ISigner} [options.signer] - Signer object
   ...
   *   object to use).
   *
   * @param {IVerifier} [options.verifier] - Verifier
   ...
   * @param documentLoader {function}
   *
   * @returns {Promise<IVerificationMethod>} The resolved verification method.
   */
  ...
  /**
   * @param verifyData {Uint8Array}.
   * @param verificationMethod {IVerificationMethod}.
   * @param document {object} to be signed.
   * @param proof {object}
   * @param documentLoader {function}
   * @param expansionMap {function}
 ...
 *
 * @returns {{verificationMethod: string, key: AbstractKeyPair,
 *   signer: ISigner, verifier: IVerifier}} - Validated and initialized
